...
@section('title', __('Edit Portfolio Item'))

@section('content')
<div class="ui-fade-in max-w-4xl space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div class="min-w-0">
            <h1 class="truncate text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">{{ __('Edit Portfolio Item') }}</h1>
            <div class="ui-muted mt-1">{{ __('Update your portfolio item') }}</div>
        </div>
    ...
    <form action="{{ localized_route('tasker.portfolio.update', $portfolioItem->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="ui-card-body">
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $portfolioItem->title) }}" required class="ui-input" placeholder="{{ __('e.g., Modern Kitchen Renovation') }}">
                    </div>

                    <div>
                        <label for="description" class="ui-label">{{ __('Description (French)') }} <span class="text-rose-600">*</span></label>
                        <textarea name="description" id="description" rows="4" required class="ui-input" placeholder="{{ __('Describe the project, what you did, challenges faced, and results achieved...') }}">{{ old('description', $portfolioItem->description) }}</textarea>
                    </div>

                    <div>
                        <label for="description_ar" class="ui-label">{{ __('Description (Arabic)') }}</label>
                        <textarea name="description_ar" id="description_ar" rows="4" class="ui-input" dir="rtl" placeholder="{{ __('وصف المشروع باللغة العربية...') }}">{{ old('description_ar', $portfolioItem->description_translations['ar'] ?? '') }}</textarea>
                    </div>

                    @if($portfolioItem->image_path)
                        <div>
                            <div class="ui-label">{{ __('Current Image') }}</div>
                            <div class="mt-2 overflow-hidden rounded-2xl border border-slate-200/70 bg-slate-100 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/40">
                                <img src="{{ $portfolioItem->getImageUrl() }}" alt="{{ $portfolioItem->image_alt ?? $portfolioItem->title }}" class="h-40 w-full object-cover" loading="lazy" decoding="async">
                            </div>
                        </div>
                    @endif

                    <div>
                        <label for="image" class="ui-label">
                            {{ $portfolioItem->image_path ? __('Replace Image') : __('Project Image') }}
                            @if(!$portfolioItem->image_path) <span class="text-rose-600">*</span> @endif
                        </label>
                        <div class="rounded-2xl border border-dashed border-slate-200/70 bg-white/50 p-6 text-center shadow-sm backdrop-blur transition hover:bg-white/70 dark:border-slate-800/70 dark:bg-slate-950/30 dark:hover:bg-slate-950/40">
                            <input type="file" name="image" id="image" accept="image/*" {{ !$portfolioItem->image_path ? 'required' : '' }} class="hidden" onchange="previewImage(this)">
                            <label for="image" class="cursor-pointer">
                                ...
                                <div id="image-placeholder">
                                    ...
                                    <div class="mt-3 text-sm font-semibold text-slate-900 dark:text-white">
                                        {{ $portfolioItem->image_path ? __('Click to replace image') : __('Click to upload an image') }}
                                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="image_alt" id="image_alt" value="{{ old('image_alt', $portfolioItem->image_alt) }}" class="ui-input" placeholder="{{ __('Describe the image for accessibility') }}">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="ui-card-body">
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <select name="category_id" id="category_id" class="ui-input">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $portfolioItem->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="task_id" id="task_id" class="ui-input">
                            ...
                            @foreach($userTasks as $task)
                                <option value="{{ $task->id }}" {{ old('task_id', $portfolioItem->task_id) == $task->id ? 'selected' : '' }}>{{ $task->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="tags" id="tags" value="{{ old('tags', $portfolioItem->tags ? implode(', ', $portfolioItem->tags) : '') }}" class="ui-input" placeholder="{{ __('e.g., renovation, kitchen, modern (separate with commas)') }}">
            ...
            <div class="ui-card-body">
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <select name="status" id="status" class="ui-input">
                            <option value="active" {{ old('status', $portfolioItem->status) == 'active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                            <option value="draft" {{ old('status', $portfolioItem->status) == 'draft' ? 'selected' : '' }}>{{ __('Draft') }}</option>
                            <option value="inactive" {{ old('status', $portfolioItem->status) == 'inactive' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                        </select>
                    </div>

                    <div>
                        <label for="display_order" class="ui-label">{{ __('Display Order') }}</label>
                        <input type="number" name="display_order" id="display_order" min="0" value="{{ old('display_order', $portfolioItem->display_order) }}" class="ui-input" placeholder="0">
                ...
                <div class="mt-5">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $portfolioItem->is_featured) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-400/40 dark:border-slate-700 dark:bg-slate-950 dark:text-white">
